...
const unsigned long measurementInterval = 60000; // 1 minuto en milisegundos
...
void processCommand(String commandData);
void calibratePH();
void calibrateDO();
void calibrateTDS();
  ...
  if (strcmp(commandType, "Stop") == 0) {
    ...
    if (sensorType != nullptr) {
      if (strcmp(sensorType, "pH") == 0) {
        ...
        Serial.println("Iniciando calibración del sensor de TDS.");
        calibrateTDS();
      } else {
...
void calibrateTDS() {
  Serial.println("Iniciando calibración del sensor TDS.");
  Serial.println("Por favor, coloque el sensor en la solución de calibración de 500ppm.");
  Serial.println("La solución debe estar a 25°C.");
  Serial.println("Presione 'Enter' para comenzar la calibración.");
  ...
  Serial.println("cal: 500");
  Serial.println("exit");
  delay(3000); // Esperar un momento para estabilizar

  // Realizar lectura de TDS en solución de calibración
  gravityTds.setTemperature(25.0); // Asegurar que la temperatura es 25°C
  gravityTds.update();
  float tdsCal = gravityTds.getTdsValue();

  // Calcular nuevo valor K
  float kValueTemp = gravityTds.getKvalue() * (500.0 / tdsCal); // 500ppm es el valor conocido
}
