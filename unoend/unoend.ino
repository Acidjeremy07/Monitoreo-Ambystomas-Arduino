...
#define TempSensorPin 3      // Sensor de Temperatura (OneWire) cambiado al pin digital 3
...
// Función para enviar el JSON al Azure IoT Hub
void sendJSON(String jsonData) {
  String topic = "devices/" + String(deviceId) + "/messages/events/";
  if (client.publish(topic.c_str(), jsonData.c_str())) {
...
void connectToAzure() {
  ...
  while (!client.connected()) {
    ...
    if (client.connect(deviceId, mqttUsername.c_str(), sasToken.c_str())) {
      ...
      String commandsTopic = "devices/" + String(deviceId) + "/messages/devicebound/#";
...
void messageReceived(char* topic, byte* payload, unsigned int length) {
  ...
  // Procesar el comando
  processCommand(commandData, false);
}
